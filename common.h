...
template <typename T>
void pureBubbleSort(QList<T>& list, bool (*compare)(const T&, const T&))
{
    int n = list.size();
    for (int i = 0; i < n - 1; i++)
    {
...
inline void pureBubbleSort(QList<QStringList>& list, int fieldIndex)
{
    int n = list.size();
    for (int i = 0; i < n - 1; i++)
    {
...
{
    static QString formatCrime(const QStringList& parts)
    {
        if (parts.size() < 5) return "";
        return "Case ID   : " + parts[0].trimmed() + "\n"
               "Location  : " + parts[1].trimmed() + "\n"
               "Type      : " + parts[2].trimmed() + "\n"
               "Desc      : " + parts[3].trimmed() + "\n"
               "Status    : " + parts[4].trimmed() + "\n"
               "----------------------------------\n";
    }

    static QString formatOfficer(const QStringList& parts)
    {
        if (parts.size() < 4) return "";
        QString assigned = (parts.size() >= 5 && !parts[4].trimmed().isEmpty()) ? parts[4].trimmed() : "Unassigned";
        return "  Officer ID : " + parts[0].trimmed() + "\n"
               "  Name       : " + parts[1].trimmed() + "\n"
               "  Rank       : " + parts[2].trimmed() + "\n"
               "  Badge #    : " + parts[3].trimmed() + "\n"
               "  Assigned   : " + assigned + "\n"
               "  --------------------------------------------------\n";
    }
    ...
    static QString formatStatus(const QStringList& parts)
    {
        if (parts.size() < 5) return "";
        return "  Case ID  : " + parts[0].trimmed() + "\n"
               "  Location : " + parts[1].trimmed() + "\n"
    ...
    {
        if (p.size() < 5) return "";
        return "  Report ID : " + p[0].trimmed() + "\n"
               "  User      : " + p[1].trimmed() + "\n"
               "  Case Ref  : " + p[2].trimmed() + "\n"
               "  Subject   : " + p[3].trimmed() + "\n"
               "  Date/Time : " + p[4].trimmed() + "\n"
               "  --------------------------------------------------\n";
    }
};
...
public:
    ...
    {
        QStringList paths;
        paths << QCoreApplication::applicationDirPath() + "/" + soundFile;
        paths << QCoreApplication::applicationDirPath() + "/../" + soundFile;
        paths << QCoreApplication::applicationDirPath() + "/../../" + soundFile;
...
{
private:
    QString soundFile = "normalButtonClick.mp3";
public:
...
public:
        ...
        : QPushButton(text, parent)
    {
        setGeometry(x, y, w, h);
        setCursor(Qt::PointingHandCursor);
        connect(this, &QPushButton::clicked, []() { SoundManager::play("normalButtonClick.mp3"); });
...
public:
        ...
        : QTextEdit(parent)
    {
        ...
        setStyleSheet(
            ...
            "QScrollBar:vertical { background: rgba(0, 15, 45, 0.80); width: 10px; margin: 0px; }"
            "QScrollBar::handle:vertical { background: #00eaff; min-height: 20px; border-radius: 5px; }"
...
public:
    RecordDisplay(int x, int y, int w, int h, QWidget* parent, const QString& placeholder = "") : QScrollArea(parent)
    {
        ...
        setStyleSheet(
            ...
            "    border-radius : 15px;"
            "}"
            "QScrollBar:vertical { background:rgba(0,15,45,0.80); width:10px; border-radius:5px; }"
            "QScrollBar::handle:vertical { background:#00eaff; min-height:20px; border-radius:5px; }"
            "QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { height:0px; }"
        );
...
struct UI
{
    static QLabel* setupBackground(QWidget* p, const QString& fileName)
    {
        QLabel* bg = new QLabel(p);
        bg->setGeometry(0, 0, 1600, 900);
        bg->setPixmap(loadPixmap(fileName));
    ...
    static void setupSidebar(QWidget* p, const QString& tagText)
    {
        new Card(0, 0, 320, 1000, p);
    ...
    static void setupPageHeader(QWidget* p, const QString& titleText, const QString& subText)
    {
        QLabel* title = new QLabel(titleText, p);
        ...
        QLabel* sub = new QLabel(subText, p);
    ...
    static void setupFooter(QWidget* p)
    {
        QLabel* footer = new QLabel("DEPARTMENT OF CRIMINAL JUSTICE    INTERNAL USE ONLY", p);
