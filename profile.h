...
// =============================================================================
//  PROFILE PAGE   (backgroundThree.png)
// =============================================================================
...
public:
    ...
    ProfilePage()
    {
        ...
        // Back Button
        Button *backBtn = new Button("  <- Back", 25, startY + 130, buttonWidth, buttonHeight, this);

        // Logout Button
        Button *logoutBtn = new Button("  Logout", 25, 840, buttonWidth, buttonHeight, this);
        logoutBtn->setStyleSheet
            (
                "QPushButton         { background:rgba(80,0,0,0.70); color:#ff9999; font-family:'Courier New',monospace; font-size:13px; font-weight:bold; letter-spacing:3px; border:1px solid rgba(255,80,80,0.45); border-radius:6px; text-align:left; padding-left:14px; }"
        ...
        feedback->setGeometry(400, 674, 820, 22);
        ...
        // Save Button
        saveBtn = new Button("  SAVE CHANGES", 400, 706, 200, 44, this);

        // Delete Button
        deleteBtn = new Button("  DELETE ACCOUNT", 1000, 706, 220, 44, this);
        deleteBtn->setStyleSheet
            (
                "QPushButton         { background:rgba(120,0,0,0.60); color:#ffcccc; font-family:'Courier New',monospace; font-size:13px; font-weight:bold; letter-spacing:2px; border:1px solid rgba(255,80,80,0.50); border-radius:6px; text-align:left; padding-left:20px; }"
    ...
    void setUserName(const QString  &name)  { nameBox->setText(name);   }
    void setUserEmail(const QString &email) { emailBox->setText(email); }

    QString getUserName()        const { return nameBox->text().trimmed();  }
    QString getUserEmail()       const { return emailBox->text().trimmed(); }
    QString getCurrentPassword() const { return currentPassBox->text();     }
    QString getNewPassword()     const { return newPassBox->text();         }

    Button *getSaveButton()   { return saveBtn;   }
    Button *getDeleteButton() { return deleteBtn; }
